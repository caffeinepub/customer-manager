...
// ─── Line Item ────────────────────────────────────────────────
type LineItemType = "labor" | "material" | "fee" | "discount" | "other";
...
function ImportJobDialog({
  ...
  function handleImport() {
    ...
    // Add base job cost if no services or cost > 0
...
function CreateInvoiceSheet({
  ...
  const importJobLineItems = useCallback((items: LineItem[]) => {
    setLineItems((prev) => {
      // Remove the placeholder blank item if it's the only one and empty
      const hasOnlyBlank =
        prev.length === 1 &&
        prev[0].description === "" &&
        prev[0].unitPrice === 0;
